...
#define ARG_FI_TYPE	    "-fitype"    /* number of restarts */
...
static int req_fi_type = RS_FI_CRASH;
...
static void print_usage(char *app_name, char *problem) 
{
  ...
  fprintf(stderr, "    %s fi <label> [%s <fi_type>]\n", app_name, ARG_FI_TYPE);
  ...
  if (req_nr == RS_UP || req_nr == RS_UPDATE || req_nr == RS_EDIT) {
	...
	*rss_flags |= RSS_NO_BIN_EXP;
	...
	/* Set default recovery script for RUN */
      ...
      {
        for (i=optind+MIN_ARG_COUNT+1; i<argc; i=i+2) {
          if (strcmp(argv[i], ARG_FI_TYPE)==0) {
                  req_fi_type = strtol(argv[i+1], NULL, 10);
                  if (req_fi_type < 0) {
                            print_usage(argv[ARG_NAME], "bad fi argument");
                            exit(EINVAL);
                  }
          }
        }
...
int main(int argc, char **argv)
{
  ...
  case RS_FI:
      ...
      m.m_rs_req.subtype = req_fi_type;
      break;
  case RS_SYSCTL:
