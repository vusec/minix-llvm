...
/* keep hypermem_entry_t in sync with hypermem-api.h */
#define HYPERMEM_STATE_SIZE_MAX 16
struct hypermem_state;

struct sef_fi_callbacks {
    size_t state_size;
    int (* hypermem_connect)(struct hypermem_state *state);
    void (* hypermem_disconnect)(struct hypermem_state *state);
    uint32_t (* hypermem_get_cr3)(void);
    hypermem_entry_t (* hypermem_read)(struct hypermem_state *state);
    void (* hypermem_write)(struct hypermem_state *state,
...
int sef_cb_init_restart_holes(int type, sef_init_info_t *info);
...
#define SEF_CB_INIT_RESTART_STTFL_HOLES	sef_cb_init_restart_holes
...
#define SEF_LU_DATA_HOLES      	     0x20000    /* The component has unpaged ranges in its data section */
...
#define SEF_SIGNAL_HANDLE_DEFAULT(S) do {         \
        extern __attribute__((weak)) void sef_signal_handle_default_hook(int); \
        if (sef_signal_handle_default_hook) \
            sef_signal_handle_default_hook(S); \
    } while(0)
 ...
 *===========================================================================*/
/* What to intercept. */
#define INTERCEPT_SEF_METADATA_REQUESTS 1
#define SEF_METADATA_REQUEST_TYPE COMMON_REQ_METADATA
#define IS_SEF_METADATA_REQUEST(mp, status) \
    ((mp)->m_type == COMMON_REQ_METADATA)

/* Debug. */
#define SEF_METADATA_DEBUG_DEFAULT 0

#ifndef SEF_METADATA_DEBUG
#define SEF_METADATA_DEBUG                  SEF_METADATA_DEBUG_DEFAULT
#endif
 
#define sef_metadata_dprint                 sef_dprint
#define sef_metadata_debug_begin            sef_debug_begin
#define sef_metadata_debug_end              sef_debug_end
