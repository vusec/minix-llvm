...
EXTERN int _ipcf_nr;
#define IPCF_EL_CHECK(E) ((((E)->flags & IPCF_MATCH_M_TYPE) || ((E)->flags & IPCF_MATCH_M_SOURCE)) \
    && (!((E)->flags & IPCF_MATCH_M_SOURCE) || IPCF_IS_ANY_EP((E)->m_source) || isokendpt((E)->m_source, &_ipcf_nr)))
...
#define IPCF_EL_MATCH_M_TYPE(E,M) (!((E)->flags & IPCF_MATCH_M_TYPE) || (E)->m_type == (M)->m_type)
#define IPCF_EL_MATCH_M_SOURCE(E,M) (!((E)->flags & IPCF_MATCH_M_SOURCE) || (E)->m_source == (M)->m_source || IPCF_EL_MATCH_M_SOURCE_ANY_EP((E)->m_source,(M)->m_source))
#define IPCF_EL_MATCH_M_SOURCE_ANY_EP(ES,MS) (((ES) == ANY_USR && IPCF_IS_USR_EP(MS)) \
    || ((ES) == ANY_SYS && IPCF_IS_SYS_EP(MS)) \
    || ((ES) == ANY_TSK && IPCF_IS_TSK_EP(MS)))
#define IPCF_EL_MATCH(E,M) (IPCF_EL_MATCH_M_TYPE(E,M) && IPCF_EL_MATCH_M_SOURCE(E,M))
...
#define IPCF_POOL_ALLOCATE_SLOT(T,S) do{ \
        int i; \
        *(S) = NULL; \
