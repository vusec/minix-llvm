...
/* Process initialization. */
  ...
  gid = cpf_grant_direct(sef_self_endpoint, 0, ULONG_MAX, CPF_READ|CPF_WRITE);
  if(!GRANT_VALID(gid)) {
...
/*===========================================================================*
 *      	           sef_cb_init_restart_holes                         *
 *===========================================================================*/
int sef_cb_init_restart_holes(int type, sef_init_info_t *info)
{
  /* We just need to set a flag. */
  info->flags |= SEF_LU_DATA_HOLES;
