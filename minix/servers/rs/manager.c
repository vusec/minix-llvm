...
static int run_script(struct rproc *rp); /* depends on VM and PM */

/*===========================================================================*
 *				caller_is_root				     *
 *===========================================================================*/
static int caller_is_root(endpoint) /* depends on PM */
endpoint_t endpoint;				/* caller endpoint */
{
  uid_t euid;

  /* Check if caller has root user ID. */
  euid = RSCHECK_ERR(getnuid(endpoint)); /* sendrec to PM */
 ...
 *===========================================================================*/
int check_call_permission(caller, call, rp) /* depends on PM */
endpoint_t caller;
int call;
struct rproc *rp;
{
/* Check if the caller has permission to execute a particular call. */
  ...
  call_allowed = caller_is_root(caller); /* depends on PM */
  if(rp) {
 ...
 *			      init_state_data				     *
 *===========================================================================*/
int init_state_data(endpoint_t src_e, int prepare_state, /* depends on DS */
  ...
  for(i=0;i<num_ipc_filters;i++) {
      ...
      for(j=0;j<IPCF_MAX_ELEMENTS && rs_ipc_filter[j].flags;j++) {
          ...
          if(flags & IPCF_MATCH_M_SOURCE) {
              if(RSCHECK_ERR(ds_retrieve_label_endpt(rs_ipc_filter[j].m_label,&m_source)) != OK) { /* sendrec to DS */
...
void end_srv_init(struct rproc *rp) /* depends on VM, PM and DS */
{
  ...
  if(rp->r_prev_rp) {
      ...
      cleanup_service(rp->r_prev_rp); /* depends on VM, PM and DS */
      rp->r_prev_rp = NULL;
      rp->r_restarts += 1;
 ...
 *===========================================================================*/
void cleanup_service_debug(file, line, rp) /* depends on VM, PM and DS */
char *file;
int line;
struct rproc *rp;
{
  ...
  if(!detach) {
      ...
      if(rp->r_pid == -1) {
          ...
          RSCHECK_ERR(srv_kill(rp->r_pid, SIGKILL)); /* sendrec to PM */
      }
  }
  ...
  if(cleanup_script) {
      ...
      s = run_script(rp); /* depends on VM and PM */
      if(s != OK) {
  ...
  if(detach) {
      ...
      detach_service(rp); /* depends on DS */
  }
  else {
 ...
 *===========================================================================*/
void detach_service_debug(file, line, rp) /* depends on DS */
char *file;
int line;
struct rproc *rp;
{
/* Detach the given system service. */
  ...
  RSCHECK_ERR(ds_publish_label(rpub->label, rpub->endpoint, DSF_OVERWRITE)); /* sendrec to DS */
 ...
 *				create_service				     *
 *===========================================================================*/
int create_service(rp) /* depends on VM, PM and DS */
struct rproc *rp;
{
/* Create the given system service. */
...
#if 0
  printf("RS: create_service: need_replica %d, need_copy %d\n",
		!!(rpub->sys_flags & SF_NEED_REPL),
		!!(rpub->sys_flags & SF_NEED_COPY));
#endif
  ...
  child_pid= RSCHECK_ERR(srv_fork(rp->r_uid, 0));	/* Force group to operator for now */ /* sendrec to PM */
  if(child_pid < 0) {
  ...
  if ((s = getprocnr(child_pid, &child_proc_nr_e)) != 0) { /* sendrec to PM */
	RSCHECK_ERR(s);
  ...
  if ((s = sys_privctl(child_proc_nr_e, SYS_PRIV_SET_SYS, &rp->r_priv)) != OK
	...
	RSCHECK_ERR(vm_memctl(RS_PROC_NR, VM_RS_MEM_PIN, 0, 0)); /* sendrec to VM */
	return ENOMEM;
  }
   ...
   */
  RSCHECK_INT(setuid(0)); /* sendrec to PM */
  ...
  if(rp->r_priv.s_flags & ROOT_SYS_PROC) {
      ...
      s = vm_memctl(rpub->endpoint, VM_RS_MEM_PIN, 0, 0); /* sendrec to VM */
      if(s != OK) {
	  RSCHECK_ERR(s);
  ...
  if(rp->r_priv.s_flags & VM_SYS_PROC) {
      ...
      s = vm_memctl(rpub->endpoint, VM_RS_MEM_MAKE_VM, 0, 0); /* sendrec to VM */
      if(s != OK) {
	  RSCHECK_ERR(s);
          printf("vm_memctl failed: %d\n", s);
          cleanup_service(rp); /* depends on VM, PM and DS */
      ...
      for(i=0;i<nr_rs_rps;i++) {
          RSCHECK_ERR(vm_memctl(rs_rps[i]->r_pub->endpoint, VM_RS_MEM_PIN, 0, 0)); /* sendrec to VM */
      }
  }

  /* Tell VM about allowed calls. */
  if ((s = vm_set_priv(rpub->endpoint, &rpub->vm_call_mask[0], TRUE)) != OK) { /* sendrec to VM */
      RSCHECK_ERR(s);
...
/* Clone the given system service instance. */
  ...
  if(rp->r_pub->endpoint == VM_PROC_NR && instance_flag == LU_SYS_PROC
      && rp->r_next_rp) {
      cleanup_service_now(rp->r_next_rp); /* depends on VM, PM and DS */
      rp->r_next_rp = NULL;
  }
  ...
  *replica_link = rp;
  ...
  r = create_service(replica_rp); /* depends on VM, PM and DS */
  if(r != OK) {
      *rp_link = NULL;
      return r;
  }
 ...
 *				publish_service				     *
 *===========================================================================*/
int publish_service(rp) /* depends on PM, VFS, DS, devman and PCI */
...
/* Publish a service. */
  ...
  r = ds_publish_label(rpub->label, rpub->endpoint, DSF_OVERWRITE); /* sendrec to DS */
  if (r != OK) {
      RSCHECK_ERR(r);
  ...
  if (rpub->dev_nr > 0) {
      ...
      if ((r = mapdriver(rpub->label, rpub->dev_nr)) != OK) { /* sendrec to VFS */
          RSCHECK_ERR(r);
  ...
  if(rpub->pci_acl.rsp_nr_device || rpub->pci_acl.rsp_nr_class) {
      ...
      r = pci_set_acl(&pci_acl); /* sendrec to DS and PCI */
      if (r != OK) {
	  RSCHECK_ERR(r);
  ...
  if (rpub->devman_id != 0) {
	  r = ds_retrieve_label_endpt("devman",&ep); /* sendrec to DS */
	  ...
	  r = ipc_sendrec(ep, &m); /* sendrec to devman */
 ...
 *			      unpublish_service				     *
 *===========================================================================*/
int unpublish_service(rp) /* depends on DS, devman and PCI */
struct rproc *rp;				/* pointer to service slot */
{
/* Unpublish a service. */
  struct rprocpub *rpub;
  ...
  printf("RS: delete %s\n", rpub->label);
  r = ds_delete_label(rpub->label); /* sendrec to DS */
  RSCHECK_ERR(r);
  ...
  if(rpub->pci_acl.rsp_nr_device || rpub->pci_acl.rsp_nr_class) {
      r = pci_del_acl(rpub->endpoint); /* sendrec to DS and PCI */
      RSCHECK_ERR(r);
      if (r != OK && !shutting_down) {
 ...
 *				start_service				     *
 *===========================================================================*/
int start_service(struct rproc *rp, int init_flags) /* depends on VM, PM, VFS, DS, devman and PCI */
{
/* Start a system service. */
  ...
  r = create_service(rp); /* depends on VM, PM and DS */
  if(r != OK) {
      return r;
  }
  ...
  if(strcmp(rp->r_pub->label, "ds")) {
	r = publish_service(rp); /* depends on PM, VFS, DS, devman and PCI */
	if (r != OK) {
		return r;
	}
  }
 ...
 *			      reincarnate_service			     *
 *===========================================================================*/
void reincarnate_service(struct rproc *old_rp) /* depends on VM, PM, VFS, DS, devman and PCI */
{
/* Restart a service as if it were never started before. */
  ...
  start_service(rp, SEF_INIT_FRESH); /* depends on VM, PM, VFS, DS, devman and PCI */
...
/* Handle a termination event for a system service. */
  ...
  if(rp->r_flags & RS_INITIALIZING) {
      ...
      if(SRV_IS_UPDATING(rp)) {
          ...
          end_update(rp->r_init_err, RS_REPLY); /* depends on VM, PM and DS */
  ...
  if(RUPDATE_IS_UPDATING()) {
      ...
      abort_update_proc(ERESTART); /* depends on VM, PM and DS */
  }
  ...
  if (rp->r_flags & RS_EXITING) {
      ...
      if(SRV_IS_UPD_SCHEDULED(rp)) {
          ...
          abort_update_proc(EDEADSRCDST); /* depends on VM, PM and DS */
      }
      ...
      unpublish_service(rp); /* depends on DS, devman and PCI */

      /* Cleanup all the instances of the service. */
      get_service_instances(rp, &rps, &nr_rps);
      for(i=0;i<nr_rps;i++) {
          cleanup_service(rps[i]); /* depends on VM, PM and DS */
      }
      ...
      if (rp->r_flags & RS_REINCARNATE) {
          ...
          reincarnate_service(rp); /* depends on VM, PM, VFS, DS, devman and PCI */
      }
  }
  else if(rp->r_flags & RS_REFRESHING) {
      /* Restart service. */
      restart_service(rp); /* depends on VM, PM and DS */
  }
  else {
...
static int run_script(struct rproc *rp) /* depends on VM and PM */
{
	...
	pid= RSCHECK_INT(fork()); /* sendrec to PM */
	switch(pid)
	{
	case -1:
		return errno;
	...
	default:
		...
		if ((r = getprocnr(pid, &endpoint)) != 0) { /* sendrec to PM */
			RSCHECK_ERR(r);
		...
		RSCHECK_ERR(vm_set_priv(endpoint, NULL, FALSE)); /* sendrec to VM */
		if ((r = vm_set_priv(endpoint, NULL, FALSE)) != OK) { /* sendrec to VM */
			RSCHECK_ERR(r);
...
/* Restart service via a recovery script or directly. */
  ...
  /* RS is not ready for anybody crashing. */
  rs_set_vulnerable(1);
  ...
  if (rp->r_script[0] != '\0') {
      r = run_script(rp); /* depends on VM and PM */
      if(r != OK) {
  ...
  if(rp->r_next_rp == NULL) {
      /* Create the replica. */
      r = clone_service(rp, RST_SYS_PROC, 0); /* depends on VM, PM and DS */
      if(r != OK) {
  ...
  r = update_service(&rp, &replica_rp, RS_SWAP, 0); /* depends on VM */
  if(r != OK) {
...
struct rproc *rp;
{
  ...
  r= stat(e_name, &sb); /* sendrec to VFS */
  if (r != 0) 
      return -errno;
  ...
  fd= RSCHECK_INT(open(e_name, O_RDONLY)); /* sendrec to VFS */
  if (fd == -1)
      return -errno;
  ...
  if (rp->r_exec == NULL)
  {
      printf("RS: read_exec: unable to allocate %zu bytes\n",
          rp->r_exec_len);
      RSCHECK_INT(close(fd)); /* sendrec to VFS */
      return ENOMEM;
  }

  r= RSCHECK_INT(read(fd, rp->r_exec, rp->r_exec_len)); /* sendrec to VFS */
  e= errno;
  RSCHECK_INT(close(fd)); /* sendrec to VFS */
  if (r == rp->r_exec_len)
      return OK;
