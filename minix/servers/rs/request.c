 ...
 *				   do_up				     *
 *===========================================================================*/
int do_up(m_ptr) /* depends on VM, PM, VFS, DS, devman and PCI */
...
/* A request was made to start a new system service. */
  ...
  if((r = check_call_permission(m_ptr->m_source, RS_UP, NULL)) != OK) /* depends on PM */
      return r;
  ...
  r = start_service(rp, init_flags); /* depends on VM, PM, VFS, DS, devman and PCI */
  if(r != OK) {
      return r;
  }
 ...
 *				do_down					     *
 *===========================================================================*/
int do_down(message *m_ptr) /* depends on VM, PM, DS, devman and PCI */
{
  ...
  if ((hypermem_notify_stop (rp->r_cmd, 0) == -1) && rs_verbose)
      printf("RS: Failed to notify hypermem of the termination\n");

  /* Check if the call can be allowed. */
  if((s = check_call_permission(m_ptr->m_source, RS_DOWN, rp)) != OK) /* depends on PM */
      return s;
 ...
 *				do_restart				     *
 *===========================================================================*/
int do_restart(message *m_ptr) /* depends on VM, PM and DS */
{
  struct rproc *rp;
  int s, r;
  char label[RS_MAX_LABEL_LEN];
  ...
  /* Check if the call can be allowed. */
  if((r = check_call_permission(m_ptr->m_source, RS_RESTART, rp)) != OK) /* depends on PM */
      return r;
 ...
 *				do_clone				     *
 *===========================================================================*/
int do_clone(message *m_ptr) /* depends on VM, PM and DS */
{
  struct rproc *rp;
  struct rprocpub *rpub;
  int s, r;
  ...
  /* Check if the call can be allowed. */
  if((r = check_call_permission(m_ptr->m_source, RS_CLONE, rp)) != OK) /* depends on PM */
      return r;
 ...
 *				do_unclone				     *
 *===========================================================================*/
int do_unclone(message *m_ptr) /* depends on VM, PM and DS */
{
  struct rproc *rp;
  struct rprocpub *rpub;
  int s, r;
  ...
  /* Check if the call can be allowed. */
  if((r = check_call_permission(m_ptr->m_source, RS_UNCLONE, rp)) != OK) /* depends on PM */
      return r;
 ...
 *				    do_edit				     *
 *===========================================================================*/
int do_edit(message *m_ptr) /* depends on VM, PM and DS */
{
  struct rproc *rp;
  struct rprocpub *rpub;
  struct rs_start rs_start;
  ...
  /* Check if the call can be allowed. */
  if((r = check_call_permission(m_ptr->m_source, RS_EDIT, rp)) != OK) /* depends on PM */
      return r;
  ...
  /* Update VM calls. */
  if ((r = vm_set_priv(rpub->endpoint, &rpub->vm_call_mask[0], /* sendrec to VM */
  ...
  if(rpub->sys_flags & SF_USE_REPL) {
      if(rp->r_next_rp) {
          cleanup_service(rp->r_next_rp); /* depends on VM, PM and DS */
          rp->r_next_rp = NULL;
      }
 ...
 *				do_refresh				     *
 *===========================================================================*/
int do_refresh(message *m_ptr) /* depends on PM */
{
  register struct rproc *rp;
  int s;
  char label[RS_MAX_LABEL_LEN];
  ...
  /* Check if the call can be allowed. */
  if((s = check_call_permission(m_ptr->m_source, RS_REFRESH, rp)) != OK) /* depends on PM */
      return s;
 ...
 *				do_shutdown				     *
 *===========================================================================*/
int do_shutdown(message *m_ptr) /* depends on PM */
{
  int slot_nr;
  struct rproc *rp;
  int r;
  ...
  /* Check if the call can be allowed. */
  if (m_ptr != NULL) {
      if((r = check_call_permission(m_ptr->m_source, RS_SHUTDOWN, NULL)) != OK) /* depends on PM */
          return r;
  }
 ...
 *				do_init_ready				     *
 *===========================================================================*/
int do_init_ready(message *m_ptr) /* depends on VM, PM and DS */
{
  int who_p;
  message m;
  ...
  if(SRV_IS_UPDATING(rp)) {
      ...
      if(rupdate.num_init_ready_pending == 0) {
          ...
          end_update(OK, RS_REPLY); /* depends on VM, PM and DS */
      }
  }
  else {
      ...
      end_srv_init(rp); /* depends on VM, PM and DS */
  }
 ...
 *				do_update				     *
 *===========================================================================*/
int do_update(message *m_ptr) /* depends on VM, PM and DS */
{
  struct rproc *rp;
  struct rproc *trg_rp;
  struct rproc *new_rp;
  ...
  /* Check if the call can be allowed. */
  if((s = check_call_permission(m_ptr->m_source, RS_UPDATE, rp)) != OK) /* depends on PM */
      return s;
  ...
  if(!prepare_only) {
      if(do_self_update) {
          ...
          s = clone_service(rp, LU_SYS_PROC, rpupd->init_flags); /* depends on VM, PM and DS */
          if(s != OK) {
          ...
          s = create_service(new_rp); /* depends on VM, PM and DS */
          if(s != OK) {
      ...
      if(rp->r_priv.s_flags & ROOT_SYS_PROC) {
          ...
          if(s != OK) {
              cleanup_service(new_rp); /* depends on VM, PM and DS */
              return s;
          }
      }
      ...
      if(rs_start.rss_heap_prealloc_bytes) {
          ...
          s = vm_memctl(new_rp->r_pub->endpoint, VM_RS_MEM_HEAP_PREALLOC, /* sendrec to VM */
              NULL, &len);
          if(s != OK) {
	      RSCHECK_ERR(s);
          ...
          if(rp->r_priv.s_flags & ROOT_SYS_PROC) {
              RSCHECK_ERR(vm_memctl(new_rp->r_pub->endpoint, VM_RS_MEM_PIN, 0, 0)); /* sendrec to VM */
          }
      }
      ...
      if(rs_start.rss_map_prealloc_bytes) {
          ...
          s = vm_memctl(new_rp->r_pub->endpoint, VM_RS_MEM_MAP_PREALLOC, /* sendrec to VM */
  ...
  /* Process state data. */
  s = init_state_data(m_ptr->m_source, prepare_state, &rs_start.rss_state_data, &rpupd->prepare_state_data); /* depends on DS */
  if(s != OK) {
      rupdate_upd_clear(rpupd); /* depends on VM, PM and DS */
      return s;
  }
  ...
  s = start_update_prepare(allow_retries); /* depends on VM, PM and DS */
  if(s == ESRCH) {
 ...
 *				do_upd_ready				     *
 *===========================================================================*/
int do_upd_ready(message *m_ptr) /* depends on VM, PM and DS */
{
  struct rproc *rp;
  ...
  if(result != OK) {
      ...
      end_update(result, RS_REPLY); /* depends on VM, PM and DS */
  ...
  start_update(); /* depends on VM, PM and DS */
 ...
 *				do_period				     *
 *===========================================================================*/
void do_period(m_ptr) /* depends on VM, PM and DS */
message *m_ptr;
{
  register struct rproc *rp;
  ...
  if(RUPDATE_IS_UPDATING() && !RUPDATE_IS_INITIALIZING()) {
      update_period(m_ptr); /* depends on VM, PM and DS */
  }
 ...
 *			          do_sigchld				     *
 *===========================================================================*/
void do_sigchld() /* depends on VM, PM and DS */
{
/* PM informed us that there are dead children to cleanup. Go get them. */
  ...
  while ( (pid = RSCHECK_INT(waitpid(-1, &status, WNOHANG))) != 0 ) { /* sendrec to PM */
      ...
      if(rp != NULL) {
          ...
          if(found) {
              rupdate_clear_upds(); /* depends on VM, PM and DS */
          }
      }
  }
}
 ...
 *				do_getsysinfo				     *
 *===========================================================================*/
int do_getsysinfo(m_ptr) /* depends on PM */
message *m_ptr;
{
  ...
  /* Check if the call can be allowed. */
  if((s = check_call_permission(m_ptr->m_source, 0, NULL)) != OK) /* depends on PM */
      return s;
 ...
 *				do_sysctl				     *
 *===========================================================================*/
int do_sysctl(message *m_ptr) /* depends on VM, PM and DS */
{
	...
	switch(request_type) {
		...
		case RS_SYSCTL_UPD_RUN:
			r = start_update_prepare(allow_retries); /* depends on VM, PM and DS */
			print_update_status();
			if(r != OK) {
			    if(r == ESRCH) {
			        /* We are done already. */
		...
		case RS_SYSCTL_UPD_STOP:
			r = abort_update_proc(EINTR); /* depends on VM, PM and DS */
			print_update_status();
			return r;
		break;
 ...
 *				do_fi				     *
 *===========================================================================*/
int do_fi(message *m_ptr) /* depends on PM */
{
  struct rproc *rp;
  struct rprocpub *rpub;
  int s, ri, type, r;
  ...
  /* Check if the call can be allowed. */
  if((r = check_call_permission(m_ptr->m_source, RS_FI, rp)) != OK) /* depends on PM */
      return r;

  /* Inject fault into the service as requested. */
  s = fi_service(rp, type);
