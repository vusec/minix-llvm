 ...
 *			      rupdate_clear_upds			     *
 *===========================================================================*/
void rupdate_clear_upds() /* depends on VM, PM and DS */
{
  ...
  RUPDATE_ITER(rupdate.first_rpupd, prev_rpupd, rpupd,
      if(prev_rpupd) {
          rupdate_upd_clear(prev_rpupd); /* depends on VM, PM and DS */
      }
  );
  rupdate_upd_clear(rupdate.last_rpupd); /* depends on VM, PM and DS */
  RUPDATE_CLEAR();
}
 ...
 *			      rupdate_upd_clear 			     *
 *===========================================================================*/
void rupdate_upd_clear(struct rprocupd* rpupd) /* depends on VM, PM and DS */
{
  /* Clear an update descriptor. */
  if(rpupd->rp->r_new_rp) {
      cleanup_service(rpupd->rp->r_new_rp); /* depends on VM, PM and DS */
  }
...
int srv_update(endpoint_t src_e, endpoint_t dst_e, int sys_upd_flags) /* depends on VM */
{
  ...
  if(src_e == VM_PROC_NR) {
      ...
      r = sys_update(src_e, dst_e,
       ...
       r = vm_update(src_e, dst_e, sys_upd_flags); /* sendrec to VM */
       RSCHECK_ERR(r);
   }
   else {
       if(rs_verbose)
 ...
 *				update_service				     *
 *===========================================================================*/
int update_service(src_rpp, dst_rpp, swap_flag, sys_upd_flags) /* depends on VM */
...
/* Update an existing service. */
  ...
  if(swap_flag == RS_SWAP) {
      if((r = srv_update(src_rpub->endpoint, dst_rpub->endpoint, sys_upd_flags)) != OK) { /* depends on VM */
          return r;
      }
  }
  ...
  if((r=sys_privctl(src_rp->r_pub->endpoint, SYS_PRIV_GET_PRIV,
    &src_rp->r_priv)) != OK) {
    panic("RS: update: could not update RS copies of priv of src: %d\n", r);
  }
  if((r=sys_privctl(dst_rp->r_pub->endpoint, SYS_PRIV_GET_PRIV,
    &dst_rp->r_priv)) != OK) {
 ...
 *			      rollback_service				     *
 *===========================================================================*/
void rollback_service(struct rproc **new_rpp, struct rproc **old_rpp) /* depends on VM */
{
  ...
  if((*old_rpp)->r_pub->endpoint == RS_PROC_NR) {
      ...
      assert(r == OK);
      if(me != RS_PROC_NR) {
          r = vm_update((*new_rpp)->r_pub->endpoint, (*old_rpp)->r_pub->endpoint, SF_VM_ROLLBACK); /* sendrec to VM */
          RSCHECK_ERR(r);
          if(rs_verbose)
      ...
      r = update_service(new_rpp, old_rpp, swap_flag, SF_VM_ROLLBACK); /* depends on VM */
  }
 ...
 *				update_period				     *
 *===========================================================================*/
void update_period(message *m_ptr) /* depends on VM, PM and DS */
{
  ...
  if(has_update_timed_out) {
      ...
      end_update(EINTR, RS_CANCEL); /* depends on VM, PM and DS */
  }
}
...
int start_update_prepare(int allow_retries) /* depends on VM, PM and DS */
{
  ...
  if(!rs_is_idle()) {
      ...
      if(!allow_retries) {
          abort_update_proc(EAGAIN); /* depends on VM, PM and DS */
      }
      return EAGAIN;
  }
          ...
          if(!UPD_IS_PREPARING_ONLY(rpupd)) {
              ...
              if(rpupd != rupdate.vm_rpupd && rpupd != rupdate.rs_rpupd) {
                  ...
                  if(!(rpupd->lu_flags & SEF_LU_UNSAFE)) {
                      ...
                      RSCHECK_ERR(vm_memctl(rp->r_pub->new_endpoint, VM_RS_MEM_PIN, 0, 0)); /* sendrec to VM */
                      if(rs_verbose)
 ...
 *				start_update				     *
 *===========================================================================*/
int start_update() /* depends on VM, PM and DS */
{
  ...
  RUPDATE_ITER(rupdate.first_rpupd, prev_rpupd, rpupd,
      ...
      if(!UPD_IS_PREPARING_ONLY(rpupd)) {
          ...
          r = start_srv_update(rpupd); /* depends on VM, PM and DS */
          if(r != OK) {
              return r;
          }
          if(!RUPDATE_IS_UPD_VM_MULTI() || rpupd == rupdate.vm_rpupd) {
              r = complete_srv_update(rpupd); /* depends on VM, PM and DS */
              if(r != OK) {
                  return r;
              }
          }
  ...
  if (!init_ready_pending) {
      end_update(OK, 0); /* depends on VM, PM and DS */
      return OK;
  }
  ...
  if(RUPDATE_IS_UPD_VM_MULTI()) {
      ...
      do_init_ready(&m); /* depends on VM, PM and DS */
...
int start_srv_update(struct rprocupd *rpupd) /* depends on VM, PM and DS */
{
  ...
  if(old_rp->r_pub->endpoint != RS_PROC_NR) {
      r = update_service(&old_rp, &new_rp, RS_SWAP, sys_upd_flags); /* depends on VM */
      if(r != OK) {
          end_update(r, RS_REPLY); /* depends on VM, PM and DS */
...
int complete_srv_update(struct rprocupd *rpupd) /* depends on VM, PM and DS */
{
  ...
  if(old_rp->r_pub->endpoint == RS_PROC_NR) {
      ...
      rollback_service(&new_rp, &old_rp); /* depends on VM */
...
int abort_update_proc(int reason) /* depends on VM, PM and DS */
{
  ...
  if(rupdate.flags & RS_INITIALIZING) {
      ...
      end_update(reason, RS_REPLY); /* depends on VM, PM and DS */
  }
  else {
      /* Pretend the current service under update failed to prepare. */
      end_update(reason, RS_CANCEL); /* depends on VM, PM and DS */
  }
 ...
 *			    end_update_curr				     *
 *===========================================================================*/
static void end_update_curr(struct rprocupd *rpupd, int result, int reply_flag) /* depends on VM, PM and DS */
{
  ...
  end_srv_update(rpupd, result, reply_flag); /* depends on VM, PM and DS */
}

/*===========================================================================*
 *			end_update_before_prepare			     *
 *===========================================================================*/
static void end_update_before_prepare(struct rprocupd *rpupd, int result) /* depends on VM, PM and DS */
{
 ...
 *===========================================================================*/
static void end_update_prepare_done(struct rprocupd *rpupd, int result) /* depends on VM, PM and DS */
{
  ...
  end_srv_update(rpupd, result, RS_REPLY); /* depends on VM, PM and DS */
}

/*===========================================================================*
 *			 end_update_initializing			     *
 *===========================================================================*/
static void end_update_initializing(struct rprocupd *rpupd, int result) /* depends on VM, PM and DS */
{
...
static void end_update_rev_iter(int result, int reply_flag,
    struct rprocupd *skip_rpupd, struct rprocupd *only_rpupd) /* depends on VM, PM and DS */
{
  ...
  RUPDATE_REV_ITER(rupdate.last_rpupd, prev_rpupd, rpupd,
      ...
      if(!UPD_IS_PREPARING_ONLY(rpupd)) {
          ...
          if((!skip_rpupd || rpupd != skip_rpupd) && (!only_rpupd || rpupd == only_rpupd)) {
              ...
              if(is_curr) {
                  end_update_curr(rpupd, result, reply_flag); /* depends on VM, PM and DS */
              }
              else if(is_before_prepare) {
                  end_update_before_prepare(rpupd, result); /* depends on VM, PM and DS */
              }
              else if(is_prepare_done) {
                  end_update_prepare_done(rpupd, result); /* depends on VM, PM and DS */
              }
              else {
                  assert(is_initializing);
                  end_update_initializing(rpupd, result); /* depends on VM, PM and DS */
              }
          }
      }
  );
    ...
    int result, int reply_flag) /* depends on VM, PM and DS */
{
  ...
  end_update_rev_iter(result, reply_flag, rupdate.vm_rpupd, NULL); /* depends on VM, PM and DS */
  if(rupdate.vm_rpupd) {
      end_update_rev_iter(result, reply_flag, NULL, rupdate.vm_rpupd); /* depends on VM, PM and DS */
  }
  ...
  RUPDATE_ITER(rupdate.first_rpupd, prev_rpupd, rpupd,
      ...
      if(result == OK && !UPD_IS_PREPARING_ONLY(rpupd)) {
          ...
          new_rp = rpupd->rp;
          end_srv_init(new_rp); /* depends on VM, PM and DS */
      }
  );
...
*			      end_srv_update				     *
 *===========================================================================*/
void end_srv_update(struct rprocupd *rpupd, int result, int reply_flag) /* depends on VM, PM and DS */
{
